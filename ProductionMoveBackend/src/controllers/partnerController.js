...
async function createPartner(req, res) {
  ...
  // Create partner
  try {
    const message = await partnerServices.createPartner(
      data,
...
async function refreshToken(req, res) {
  ...
  try {
    const message = await partnerServices.refreshToken(
      req.headers.authorization
    );
    return res.status(200).json(message);
  } catch (err) {
...
async function getPartnersByQuery(req, res) {
  ...
  try {
    const message = await partnerServices.findPartnersByQuery(
      data,
      req.headers.authorization
    );
    return res.status(200).json(message);
...
async function getResources(req, res) {
  ...
  try {
    const message = await partnerServices.getResources(
      data,
      req.headers.authorization
    );
    return res.status(200).json(message);
