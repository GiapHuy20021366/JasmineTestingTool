...
import { messageCreater } from "../services/untilsServices.js";
/**
 *
...
async function createProducts(req, res) {
  ...
  if (!req?.headers?.authorization) {
    return res
      .status(403)
      .json(messageCreater(-4, "error", "Missing parameters: Token not found"));
  }
  ...
  if (!req?.body?.products) {
    return res
      .status(400)
      .json(messageCreater(-3, "error", "Missing parameters"));
  }
      ...
      return res
        ...
        .json(
          messageCreater(
            -3,
            "error",
            "Length array of products must larger than 0"
          )
        );
    }
    for (let i in products) {
      if (!products[i].modelId) {
        return res
          .status(400)
          .json(
            messageCreater(-3, "error", `Missing parameters at index ${i}`)
          );
      }
      if (!products[i].birth) {
...
async function getProductsByQuery(req, res) {
  ...
  try {
    const message = await productServices.findProductsByQuery(
      data,
...
async function getCurrentLocationOfProducts(req, res) {
  ...
  try {
    ...
    if (!listId) listId = [];
    const token = req.headers.authorization;
    const message = await productServices.getCurrentLocationOfProducts(
      listId,
      token
    );
    return res.status(200).json(message);
...
async function getCurrentProductsByQuery(req, res) {
  ...
  try {
    const message = await productServices.getProductsCurrent(
      data,
      req.headers.authorization
    );
    return res.status(200).json(message);
...
async function getNewestExportsByIds(req, res) {
  ...
  try {
    const message = await productServices.getExportsByIds(
      data.productIds,
      req.headers.authorization
    );
    return res.status(200).json(message);
