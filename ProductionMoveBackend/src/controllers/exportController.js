...
async function getExportsByQuery(req, res) {
  ...
  try {
    const message = await exportServices.findExportsByQuery(
      data,
      ...
      return res
        ...
        .json(
          messageCreater(
            -3,
            "error",
            "Length array of exportIds must larger than 0"
          )
        );
    }
      ...
      return res
        ...
        .json(
          messageCreater(
            -3,
            "error",
            "Length array of productIds must larger than 0"
          )
        );
    }
    const message = await exportServices.returnProductsToCustomer(
      { productIds, note },
      token
    );
    return res.status(200).json(message);
