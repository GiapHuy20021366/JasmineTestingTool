...
import { messageCreater } from "./untilsServices.js";
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
            ...
            if (!modelIdToFactoryId[products[index].modelId]) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No model id found for product at index ${index}`
                )
              );
              return;
            }
            if (
              modelIdToFactoryId[products[index].modelId] !==
              ...
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to create product at index ${index}`
                )
              );
              return;
            }
          ...
          // Create products
          const productsDB = await db.Products.bulkCreate(products, {
            returning: true,
          });
          ...
          const productHoldersDB = await db.ProductHolders.bulkCreate(
            productHolders,
            { returning: true }
          );
          // Success created
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", "Get products successful!", productsDB)
          );
        } catch (error) {
          console.log(error);
          reject(messageCreater(-5, "error", "Database Error!"));
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", `Found ${rows.length} products`, {
              count,
              rows,
            })
          );
          ...
          if (query.partnerId !== partnerId && role !== 1) {
            reject(
              messageCreater(
                -3,
                "error",
                `Authentication failed: Not Permision`
              )
            );
            return;
          } else {
