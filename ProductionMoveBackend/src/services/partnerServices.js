const bcryptServices = require("./bcryptServices.js");
const db = require("../models/index");
const { messageCreater } = require("./untilsServices.js");
const authenticationServices = require("./authenticationServices.js");
...
async function loginPartner(partner) {
  return new Promise(async (resolve, reject) => {
    ...
    // Found partner
    if (partnerDB) {
      await bcryptServices
        .compare(partner.password, partnerDB.password)
        .then((message) => {
          ...
          resolve(
            messageCreater(1, "success", "Login successful!", {
              token,
              id: partnerDB.id,
              name: partnerDB.name,
              userName: partnerDB.userName,
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        if (message.data.data.role !== 1) {
          reject(
            messageCreater(-3, "error", `Authentication failed: Not Permission`)
          );
          return;
        }
        ...
        try {
          ...
          // Hash password
          // console.log("IM HEASJF");
          const hashPasswordMessage = await bcryptServices
          ...
          resolve(
            messageCreater(1, "success", "Create partner successful!", {
              id: newPartnerDB.id,
            })
          );
          ...
          // Send email
          ejs.renderFile(
            path.resolve(__dirname, EJS_PATH, "email-create-partner.ejs"),
            {
      ...
      .catch((error) => {
        // Token error
        reject(
          messageCreater(-3, "error", `Authentication failed: ${error.name}`)
        );
      });
  });
}
  ...
  return new Promise(async (resolve, reject) => {
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        resolve(
          messageCreater(1, "success", "Token refresh successful!", { token })
        );
      })
      .catch((error) => {
        // Token error
  ...
  return new Promise(async (resolve, reject) => {
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        if (message.data.data.role !== 1) {
          reject(
            messageCreater(-3, "error", `Authentication failed: Not Permision`)
          );
          return;
        }
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", `Found ${rows.length} partners`, {
              count,
              rows,
            })
          );
      ...
      .catch((error) => {
        // Token error
        reject(
          messageCreater(-2, "error", `Authentication failed: ${error.name}`)
        );
      });
  });
}
        ...
        if (resources?.modelAttributes) {
          ...
          modelsDB.forEach((model) => {
            ...
            if (
              ...
              !bodyTypes.includes(bodyType)
            ) {
              bodyTypes.push(bodyType);
            }
            if (
              engineType &&
              modelAttributes.engineType &&
              !engineTypes.includes(engineType)
            ) {
              engineTypes.push(engineType);
            }
            if (
              boostType &&
              modelAttributes.boostType &&
              !boostTypes.includes(boostType)
            ) {
