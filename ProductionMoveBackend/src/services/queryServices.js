 ...
 * @type {import('./untilsServices.js').Message}
 */
function parseQuery(query, model, exceptAttributes = []) {
    ...
    for (let element of op) {
      ...
      if (!validAttributes.includes(attribute)) {
        return messageCreater(
          -1,
          "error",
          `No attribute name ${attribute} at ${location} ... ${JSON.stringify(
            element
          )}`
        );
      }
  ...
  if (query.operations) {
    ...
    for (let op of Object.keys(operations)) {
      const messageInvalid = getMessageInvalidAttribute(
        operations[op],
        `operations.${op}`
      );
      ...
      switch (op) {
        ...
        case "or":
          ...
          break;
        default:
          throw messageCreater(
            -2,
            "error",
            `Invalid operator ${op} at operations`
          );
      }
    }
  }
    ...
    for (let attribute of Object.keys(attributes)) {
      if (!validAttributes.includes(attribute)) {
        throw messageCreater(
          -1,
          "error",
          `Invalid attribute ${attribute} found at attributes`
        );
      }
      where[attribute] = {};
        ...
        switch (op) {
          ...
          default:
            throw messageCreater(
              -2,
              "error",
              `No operator name ${op} at attributes.${attribute} found`
            );
        }
      }
    }
