    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", `Found ${rows.length} maintains`, {
              count,
              rows,
            })
          );
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          if (invalidIds.length > 0) {
            reject(
              messageCreater(
                -1,
                "error",
                `No permission to maintain product with ids ${JSON.stringify(
                  invalidIds
                )}`
              )
            );
            ...
            if (product.purchase.dealer.id !== thisPartnerId) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to maintain product with id ${product.id}`
                )
              );
              return;
            }
          ...
          // Save maintainDatas
          const maintainsDB = await db.Maintains.bulkCreate(maintainDatas, {
            returning: true,
          });
          ...
          resolve(
            messageCreater(
              1,
              "success",
              `Maintain products confirm successful`,
              maintainsDB
            )
          );
        } catch (error) {
