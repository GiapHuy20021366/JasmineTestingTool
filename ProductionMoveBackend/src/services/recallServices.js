    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", `Found ${rows.length} recalls`, {
              count,
              rows,
            })
          );
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
            ...
            if (holder.partner1Id !== -1 || holder.customerId === -1) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to recall product with id ${holder.productId}`
                )
              );
              return;
            }
            ...
            if (product.purchase.dealer.id !== thisPartnerId) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to recall product with id ${product.id}`
                )
              );
              return;
            }
          ...
          // Save maintainDatas
          const recallsDB = await db.Recalls.bulkCreate(recallDatas, {
            returning: true,
          });
          ...
          resolve(
            messageCreater(
              1,
              "success",
              `Recall products confirm successful`,
              recallsDB
            )
          );
        } catch (error) {
