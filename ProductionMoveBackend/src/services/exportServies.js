    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          resolve(
            messageCreater(1, "success", `Found ${rows.length} exports`, {
              count,
              rows,
            })
          );
    ...
    await authenticationServices
      ...
      .then(async (message) => {
        ...
        try {
          ...
          const fromPartnerDB = await db.Partners.findByPk(
          ...
          if (!toPartnerDB) {
            reject(
              messageCreater(
                -2,
                "error",
                `Can't find partner with id = ${toPartnerId}`
              )
            );
            return;
          }
            ...
            if (holder.partner1Id !== partnerId) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to export product with id ${holder.productId}`
                )
              );
              validAll = false;
              return;
          ...
          // Create exports
          const exportsDB = await db.Exports.bulkCreate(exports, {
            returning: true,
          });
          ...
          if (!isValidAllExport) {
            reject(
              messageCreater(
                -1,
                "error",
                `No permission to confirm export with id ${invalidExport.id}`
              )
            );
            return;
          }
            ...
            if (
              ...
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to return product with id ${holder.productId}`
                )
              );
              return;
            }
            ...
            if (!product?.purchase?.customer) {
              reject(
                messageCreater(
                  -1,
                  "error",
                  `No permission to return product with id ${product.id}`
                )
              );
              return;
            }
