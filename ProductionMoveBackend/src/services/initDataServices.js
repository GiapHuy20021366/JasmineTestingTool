...
import bcryptServices from "./bcryptServices.js";
...
const initPartners = (req, res) => {
  const results = [];
  fs.createReadStream(
    path.resolve(__filename, "../../public/data-init-samples/partners.csv")
  )
    .pipe(csv())
...
const initWarehouses = (req, res) => {
  const results = [];
  fs.createReadStream(
    path.resolve(__filename, "../../public/data-init-samples/warehouses.csv")
  )
    .pipe(csv())
    .on("data", (data) => results.push(data))
    .on("end", async () => {
...
const initModels = (req, res) => {
  const results = [];
  fs.createReadStream(
    path.resolve(__filename, "../../public/data-init-samples/models.csv")
  )
    .pipe(csv())
    .on("data", (data) => results.push(data))
    .on("end", async () => {
